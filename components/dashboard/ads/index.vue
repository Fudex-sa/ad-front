...
<div class="col-md-12">
    ...
    <div class="div-top">
        <nuxt-link :to="{ name: 'advertiser-dashboard-ads-new' }" class="the-btn hvr-radial-out" v-if="role == 'advertiser'">Create New</nuxt-link>
                    ...
                    <td class="hidden-sm">
                        ...
                        <div class="text-center">
                            <template v-if="user.role == 'advertiser'">
                                <!-- Delete -->
                                <button class="fa fa-trash-o fa-lg btn btn-danger action-btn" @click="deleteAd(ad.id, index)"></button>
                                <!-- Edit -->
                                <nuxt-link class="btn btn-info action-btn" :to="{
...
params: { id: ad.id },
                            ...
                            <nuxt-link class="btn btn-info action-btn" :to="{name: 'soldier-dashboard-ads-id',params: { id: ad.id },}"> View</nuxt-link>
                            <!-- Reviewing -->
                            <button class="btn btn-success action-btn" @click="activateAd(ad.id)" v-if="user.role === 'admin' && ad.status == 'reviewing'">
                                Activate
                            </button>
                            <!-- Statistics -->
                            <nuxt-link class="btn action-btn btn-primary" :to="{

name: 'advertiser-dashboard-ads-id-statistics',

params: { id: ad.id },

}" v-if="ad.status == 'active'">Statistics</nuxt-link>
                        </div>
                    </td>
                </tr>
            </tbody>
