...
<div class="admin-header">
    ...
    <div class="notifications">
        ...
        <div class="notification-content">
            <ul class="m-0">
                <li :class="{ 'bg-light': !notification.read_at }" v-for="notification in notifications.data" :key="notification.id" @click="readNotification(notification.id)">
                    <div>{{ notification.data.title }}</div>
                    <div>
                        <button class="the-btn hvr-radial-out" @click="subscribeAd(notification.data.id)">
                            Subscribe
                        </button>
                        <nuxt-link class="the-btn hvr-radial-out" :to="{
